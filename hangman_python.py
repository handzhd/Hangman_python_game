...
def hangman():
    # get the letter of word and new object set for tracking letter guess
    word = get_word(words)
    ...
    while len(word_letters) > 0 and lives > 0:
        ...
        print('Correct guess: ',' '.join(sorted(correct_guess)))
        print('Wrong guess: ',' '.join(sorted(wrong_guess)))

        # word letter
        word_list = [letter if letter in correct_guess else '-' for letter in word]
        print('\nYour word: ', ' '.join(word_list))
        
        # getting user input
        user_input = input('\nGuess a letter: ').upper()
        if user_input in alpha:
            if user_input in word_letters:
                correct_guess.add(user_input)
                word_letters.remove(user_input)

            elif user_input in correct_guess or user_input in wrong_guess:
                print('\nYou have already guessed it. Please try again.')
                lives -= 1
            
            else:
                wrong_guess.add(user_input)
                lives -= 1
                print('\nWrong guessed. Please try again.')
        
        else:
            lives -= 1
            print('\nYou input wrong character. Please try again.')

    # while loop finish
    if lives == 0:
        print(hangman_visual[lives])
        print('                     You died.')
        print('You failed to guess this word: ', word, '\n')
    else:
        print(hangman_visual[lives], '           You saved me!')
        print('\nYour word: ', ' '.join(word))
        print('Congratulations! You successfully guess the word.')
